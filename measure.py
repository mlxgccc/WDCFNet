...
mea_parser.add_argument('--llvip', action='store_true', help='measure llvip dataset')
...
mea_parser.add_argument('--huawei', action='store_true', help='measure huawei dataset')
mea_parser.add_argument('--nikon', action='store_true', help='measure nikon dataset')
...
def calculate_ssim(target, ref):
    ...
    if img1.ndim == 2:
        ...
        raise ValueError('Wrong low image dimensions.')
...
def metrics(im_dir, label_dir, use_GT_mean):
    ...
    for item in tqdm(sorted(glob.glob(im_dir))):
        ...
        img_tensor = torch.from_numpy(np.float32(im1)).permute(2,0,1).unsqueeze(0).cuda()
        gt_tensor  = torch.from_numpy(np.float32(im2)).permute(2,0,1).unsqueeze(0).cuda()

        score_lpips = loss_fn(gt_tensor, img_tensor)  # gt -> input order
        avg_psnr += score_psnr
        avg_ssim += score_ssim
...
if __name__ == '__main__':
    ...
    if mea.lol:
        im_dir = r'D:\person\Low-Light\WDCFNet\results\LOLv1\*.png'
        label_dir = './datasets/LOLv1/Test/high/'
    if mea.llvip:
        im_dir = './output/*.jpg'
        label_dir = r'D:\person\Low-Light\LLVIPvalue\Test\high/'
    if mea.lol_v2_real:
        im_dir = './results/LOLv2_real/*.png'
        label_dir = './datasets/LOLv2/Real_captured/Test/groundtruth/'
    if mea.lol_v2_syn:
    ...
    if mea.SICE_mix:
        ...
        label_dir = './datasets/SICE/SICE_Reshape/'
    if mea.huawei:
        im_dir = './results/Huawei/*.jpg'
        label_dir = './datasets/Huawei/Test/high/'
    if mea.nikon:
        im_dir = './results/Nikon/*.jpg'
        label_dir = './datasets/Nikon/Test/high/'
