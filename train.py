...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        im1 = im1.cuda()
        im2 = im2.cuda()
        freq_features = dwt2_gpu(im1,save=False, folder="wavelet_output", prefix="test")
        # alpha, beta, gamma = 0.33, 0.33, 0.34
        # HF_fused = alpha * freq_features[1] + beta *  freq_features[2] + gamma * freq_features[3]

        folder = "fusion_output"
        os.makedirs(folder, exist_ok=True)


        output_rgb = model((im1, freq_features[0],freq_features[1]))
...
def load_datasets():
    ...
    if opt.lol_v1 or opt.lol_blur or opt.lolv2_real or opt.lolv2_syn or opt.SID or opt.SICE_mix or opt.SICE_grad or opt.huawei or opt.llvip or opt.nikon:
        if opt.lol_v1:
        ...
        if opt.llvip:
            train_set = get_lol_training_set(opt.data_train_llvip, size=opt.cropSize)
            training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize,
                                              shuffle=opt.shuffle)
            test_set = get_eval_set(opt.data_val_llvip)
            testing_data_loader = DataLoader(dataset=test_set, num_workers=opt.threads, batch_size=1, shuffle=False)
        ...
        if opt.huawei:
            train_set = get_huawei_training_set(opt.data_train_huawei, size=opt.cropSize)
            training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize,
                                              shuffle=opt.shuffle)
            test_set = get_eval_set(opt.data_val_huawei_grad)
            testing_data_loader = DataLoader(dataset=test_set, num_workers=opt.threads, batch_size=1, shuffle=False)
        if opt.nikon:
            train_set = get_nikon_training_set(opt.data_train_nikon, size=opt.cropSize)
            training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize,
                                              shuffle=opt.shuffle)
            test_set = get_eval_set(opt.data_val_nikon_grad)
            testing_data_loader = DataLoader(dataset=test_set, num_workers=opt.threads, batch_size=1, shuffle=False)
        ...
        if opt.start_warmup:
            scheduler_step = CosineAnnealingRestartCyclicLR(optimizer=optimizer,
                                                            periods=[(opt.nEpochs // 4) - opt.warmup_epochs,
                                                                     (opt.nEpochs * 3) // 4], restart_weights=[1],
            ...
            scheduler = CosineAnnealingRestartCyclicLR(optimizer=optimizer,
                                                       ...
                                                       restart_weights=[1], eta_mins=[0.0002, 0.0000001])
...
if __name__ == '__main__':
    ...
    # 检查当前使用的设备
    print(torch.cuda.current_device())  # 当前GPU索引（0表示第一块）
    ...
    for epoch in range(start_epoch + 1, opt.nEpochs + start_epoch + 1):
        ...
        if epoch % opt.snapshots == 0:
            ...
            if opt.huawei:
                output_folder = 'huawei/'
                label_dir = opt.data_valgt_huawei
            if opt.llvip:
                output_folder = 'llvip/'
                label_dir = opt.data_valgt_llvip
            if opt.nikon:
                output_folder = 'nikon/'
                label_dir = opt.data_valgt_nikon
